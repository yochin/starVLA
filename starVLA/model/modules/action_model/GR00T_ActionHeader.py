...
class FlowmatchingActionHeadConfig(PretrainedConfig):
    ...
    ignored_action_dims: list = field(default=None, metadata={"help": "Optional action dimension indices to ignore in loss."})
...
class FlowmatchingActionHead(nn.Module):
    def __init__(
        ...
        ignored_dims = config.get("ignored_action_dims", None) if hasattr(config, "get") else getattr(config, "ignored_action_dims", None)
        if ignored_dims is not None and len(ignored_dims) > 0:
            ...
            logger.info(f"FlowmatchingActionHead: Ignoring action dimensions {list(ignored_dims)} in loss. Active dims: {int(mask.sum().item())}/{self.action_dim}")
        else:
    ...
    def forward(
        ...
        diff2 = (pred_actions - velocity) ** 2
        if self.action_loss_mask is not None:
            loss_mask = self.action_loss_mask.to(device=diff2.device, dtype=diff2.dtype)  # (action_dim,)
            loss = (diff2 * loss_mask).sum() / (loss_mask.sum() * diff2.shape[0] * diff2.shape[1])
        else:
            loss = diff2.mean()
        if return_per_dim:
            per_dim_loss = diff2.mean(dim=(0, 1)).detach()
            return loss, per_dim_loss
        return loss

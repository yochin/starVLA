...
from deployment.model_server.tools.image_tools import prepare_multiview_images
...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        for modality in ["state", "action"]:
            ...
            if self.modality_configs and modality in self.modality_configs:
                configured_keys = {
                    k.replace(f"{modality}.", "", 1)
                    for k in self.modality_configs[modality].modality_keys
                    if k.startswith(f"{modality}.")
                }

            for subkey in le_state_action_meta:
                if configured_keys is not None and subkey not in configured_keys:
                    continue
        ...
        configured_video_keys = None
        if self.modality_configs and "video" in self.modality_configs:
        ...
        for new_key in le_modality_meta.video:
            if configured_video_keys is not None and new_key not in configured_video_keys:
                continue
    ...
    def _pack_sample(self, data: dict) -> dict:
        ...
        raw_step_images = [
            Image.fromarray(data[video_key][0])
        ...
        split_indices = (
            self.data_cfg.get("split_side_by_side_indices", [])
            if self.data_cfg is not None
            else []
        )
        letterbox_images = (
            bool(self.data_cfg.get("letterbox_images", False))
            if self.data_cfg is not None
            else False
        )
        step_images = prepare_multiview_images(
            raw_step_images,
...
class LeRobotMixtureDataset(Dataset):
    ...
    def compute_overall_statistics(
        ...
        # Get the list of modality keys present across all tasks
        common_modalities = [
            m for m in per_task_stats[0].keys()
            if all(m in task_stats for task_stats in per_task_stats[1:])
        ]
    ...
    def merge_metadata(
        ...
        for m in metadata_dicts:
            all_modalities.update(m["modalities"].keys())
        ...
        for modality in sorted(all_modalities):
            ...
            if not all_configs:
                continue
            if modality == "video":
                # For video, allow different FPS across datasets (mixed FPS mixture).
                # Verify that camera keys, resolutions, and channels match.
                base_vid = copy.deepcopy(all_configs[0])
                for vid in all_configs[1:]:
                    assert set(vid.keys()) == set(base_vid.keys()), (
                        f"Video camera keys mismatch: {set(vid.keys())} vs {set(base_vid.keys())}"
                    )
                    for k in base_vid:
                        assert vid[k]["resolution"] == base_vid[k]["resolution"], (
                            f"Video resolution mismatch for {k}: {vid[k]['resolution']} vs {base_vid[k]['resolution']}"
                        )
                        assert vid[k]["channels"] == base_vid[k]["channels"], (
                            f"Video channels mismatch for {k}: {vid[k]['channels']} vs {base_vid[k]['channels']}"
                        )
                merged_metadata["modalities"]["video"] = base_vid
            else:
                mod_json_set = {json.dumps(cfg, sort_keys=True) for cfg in all_configs}
                assert len(mod_json_set) == 1, (
                    f"Multiple modality configs for modality {modality}: {list(mod_json_set)}"
                )
                merged_metadata["modalities"][modality] = all_configs[0]

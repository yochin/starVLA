...
class VLATrainer(TrainerUtils):
    ...
    def _log_metrics(self, metrics):
        ...
        if self.completed_steps % self.config.trainer.logging_frequency == 0 and rank == 0:
            ...
            parts_str = ""
            part_keys = [k for k in metrics if k.startswith("loss_parts/")]
            if part_keys:
                parts_str = " | Parts: [" + ", ".join(f"{k.replace('loss_parts/', '')}: {metrics[k]:.4f}" for k in part_keys) + "]"

            logger.info(f"Step {self.completed_steps}, Loss: {metrics.get('action_dit_loss', 0.0):.5f}{parts_str}")
    ...
    def train(self):
        ...
        while self.completed_steps < self.config.trainer.max_train_steps:
            ...
            if self.accelerator.is_local_main_process:
                postfix = {
                    "loss": f"{step_metrics.get('action_dit_loss', 0.0):.4f}",
                }
                for p in ["R_arm", "R_hand", "L_arm", "L_hand", "legs", "waist", "head"]:
                    k = f"loss_parts/{p}"
                    if k in step_metrics:
                        postfix[p] = f"{step_metrics[k]:.4f}"
    ...
    def _train_step(self, batch_vla, batch_vlm=None):
        ...
        if "per_dim_loss" in output_dict and output_dict["per_dim_loss"] is not None:
            pdim = output_dict["per_dim_loss"].cpu().numpy()
            if len(pdim) == 45:
                metrics.update({
                    "loss_parts/head": float(pdim[0:2].mean()),
                    "loss_parts/L_arm": float(pdim[2:9].mean()),
                    "loss_parts/L_hand": float(pdim[9:16].mean()),
                    "loss_parts/legs": float(pdim[16:28].mean()),
                    "loss_parts/R_arm": float(pdim[28:35].mean()),
                    "loss_parts/R_hand": float(pdim[35:42].mean()),
                    "loss_parts/waist": float(pdim[42:45].mean()),
                })
            elif len(pdim) == 7:
                metrics.update({
                    "loss_parts/arm": float(pdim[0:6].mean()),
                    "loss_parts/gripper": float(pdim[6]),
                })
            elif len(pdim) == 14:
                metrics.update({
                    "loss_parts/L_arm": float(pdim[0:6].mean()),
                    "loss_parts/L_gripper": float(pdim[6]),

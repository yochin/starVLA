...
def prepare_multiview_images(
    images,
    target_size=(224, 224),
    split_side_by_side_indices=(),
    letterbox=False,
    method=Image.BILINEAR,
):
    """Prepare an ordered list of camera images for training and inference.

    target_size is (width, height). Inputs selected by
    split_side_by_side_indices emit left and right views in that order.
    When letterbox is true, aspect ratio is preserved with zero padding.
    """
    if not isinstance(images, (list, tuple)):
        images = [images]

    target_width, target_height = (int(target_size[0]), int(target_size[1]))
    split_indices = {int(index) for index in split_side_by_side_indices}
    prepared = []

    for index, image in enumerate(images):
        pil_image = to_pil_preserve(image)
        views = [pil_image]
        if index in split_indices:
            width, height = pil_image.size
            if width < 2:
                raise ValueError(
                    f"Cannot split side-by-side image at index {index}: size={pil_image.size}"
                )
            midpoint = width // 2
            views = [
                pil_image.crop((0, 0, midpoint, height)),
                pil_image.crop((midpoint, 0, width, height)),
            ]

        for view in views:
            if letterbox:
                view = _resize_with_pad_pil(
                    view,
                ...
                view = view.resize((target_width, target_height), resample=method)

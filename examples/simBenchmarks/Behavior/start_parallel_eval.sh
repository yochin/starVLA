...
export TASKS_JSONL_PATH=/workspace/llavavla0/examples/simBenchmarks/Behavior/tasks.jsonl
...
for task_name in "${INSTANCE_NAMES[@]}"; do   
  ...
  for ((run_idx=1; run_idx<=TEST_NUM; run_idx++)); do
      ...
      if [ $? -eq 0 ]; then
        ...
        cmd="CUDA_VISIBLE_DEVICES=${gpu_id} ${sim_python} examples/simBenchmarks/Behavior/start_behavior_env.py \
          --ckpt-path ${ckpt_path} \
